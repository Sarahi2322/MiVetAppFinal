...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // 2) Menú hamburguesa (robusto y sin tocar estilos inline)
  const btnMenu  = document.querySelector('.menu-toggle');
  const sidebar  = document.querySelector('.sidebar');
  const backdrop = document.getElementById('backdrop'); // opcional en tu HTML
  ...
  // 3) CRUD simple de Servicios (solo si existen <form> y #list)
  const form  = document.querySelector('form');
  const listEl = document.getElementById('list');

  async function loadList() {
    if (!listEl) return; // no hay lista en esta página
    try {
